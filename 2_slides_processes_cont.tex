...
\begin{frame}{Примеры случайных процессов}
    ...
    \noindent\makebox[\textwidth]{\includegraphics[width=1.0\textwidth]{1_figs/process_example_1.png}}
\end{frame}
...
\begin{frame}{Примеры случайных процессов}
    ...
    \makebox[\textwidth]{\includegraphics[width=1.0\textwidth]{1_figs/process_example_2.png}}
\end{frame}
...
\begin{frame}{Предел случайного блуждания}
    ...
    \end{itemize}
    \makebox[\textwidth]{\includegraphics[width=1.0\textwidth]{2_figs/random_walk_limit.png}}
\end{frame}
...
\begin{frame}{Броуновское движение}
    ...
    \end{block}
\end{frame}
%% TODO: поменять везде B_t на B_t
...
\begin{frame}{Вариация функции/процесса}
    ...
    \end{block} \only<1>{Для дифференцируемых функций $V_t(X)=\int_0^t |X'_t|dt$.}
    \only<2>{\begin{block}{Определение}
